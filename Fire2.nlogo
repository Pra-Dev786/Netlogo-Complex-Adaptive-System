...
to setup
  ...
  ; Rain Area:
    set oppo-xstart ( - max-pxcor / 2)
    set oppo-xend (max-pxcor / 2)
    set oppo-ystart ( - max-pxcor / 2)
    set oppo-yend ( max-pxcor / 2)
end
...
to go
  ...
  ask fires
    [
      ...
      [
        ifelse oppo and (pxcor < oppo-xend) and (pxcor > oppo-xstart) and (pycor > oppo-ystart) and (pycor < oppo-yend)
        [ set oppo-zone TRUE ]
        [ set oppo-zone FALSE ]

        ifelse oppo-zone and ((random-float 100) < oppo-chance)
        [ ]
        [ ignite ]
      ]
      ...
      if supp and (pxcor < max-pxcor - 2) and (pxcor > min-pxcor + 2) and (pycor < max-pycor - 2) and (pycor > min-pycor + 2)
     [
        ask patch-at-heading-and-distance supp-angle 3
        [
            ifelse oppo and (pxcor < oppo-xend) and (pxcor > oppo-xstart) and (pycor > oppo-ystart) and (pycor < oppo-yend)
            [ set oppo-zone TRUE ]
            [ set oppo-zone FALSE ]
